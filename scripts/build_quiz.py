...
<div class="wrap">
  ...
  <div id="result" class="card hide">
    ...
    <div class="btns">
      ...
      <button class="btn" onclick="retry()">もう一度</button>
    </div>
  </div>
...
let session=[], idx=0, score=0, sessionWrong=[], lastSession=null;
...
function showResult(){
  ...
  const rv=$('review');
  if(!sessionWrong.length){ rv.innerHTML='<p style="text-align:center">全問正解！おめでとうございます 🎉</p>'; return; }
  rv.innerHTML='<h3>復習（今回の不正解）</h3>'+sessionWrong.map(q=>
