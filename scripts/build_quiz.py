...
// 間違えた問題だけを再挑戦（B方式：その回で全問正解するまで反復）
function retryWrong(){
  const wrong=sessionWrong.slice();
...
function showTestResult(){
  ...
  if(remain.length===0){
    ...
    t.attempts++; t.last=testFirstScore; t.best=Math.max(t.best,testFirstScore); t.passed=true;
    LS.set('dq_test',t);
    saveTestResult(testFirstScore, testFirstDetail, true);
    $('final').innerHTML='🎉 修了！<small>40問すべて正解しました（初回正答 '+testFirstScore+' / 40・'+testRound+'回で達成）</small>';
    ...
    // まだ間違いあり → 間違えた問題だけ再挑戦
    $('final').innerHTML=(session.length-remain.length)+' / '+session.length+'<small>正解 — 残り '+remain.length+' 問を全問正解すると修了です</small>';
    let html='<h3>もう一度挑戦する問題（解説）</h3>'+remain.map(function(q){
      return '<div class="rev"><div class="q">［'+q.chapter+'］'+esc(q.stem)+'</div>'+
      '<div><b style="color:var(--accent)">正解：'+q.ans+'</b> '+esc(q.opts[q.ans-1])+'</div>'+
      '<div style="color:var(--mut);font-size:.85rem;margin-top:4px">'+esc(q.explanation)+'</div></div>'; }).join('');
    $('review').innerHTML=html;
    rb.style.display=''; rb.textContent='間違いを再挑戦（'+remain.length+'問）'; rb.onclick=retryWrong;
  }
}
...
function renderTestCard(){
  ...
  const s=$('testStatus');
  if(t.passed){ s.textContent='修了'; s.className='tpill ok'; }
